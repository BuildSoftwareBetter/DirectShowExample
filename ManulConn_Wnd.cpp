...
// const BSTR szVideoDeviceName = L"SEMIC Camera";
...
AM_MEDIA_TYPE * GetMediaType(IBaseFilter *pDeviceFilter, LONG pSelectWidth, LONG pSelectHeight, int pSelectFrameRate, GUID pSelectFormat , GUID pSelectVideoType )
{
	...
	pStreamConfig->SetFormat(pSelectMediaType);

	pStreamConfig->Release();
	pStreamConfig = NULL;

	return pSelectMediaType;
}
...
int ShowVideo(HWND hwnd, long lMsg, BSTR szVideoDeviceName, IMediaEventEx **opEvent, IMediaControl **opControl)
{
    ...
    *opControl = pControl;
    ...
    AM_MEDIA_TYPE *pMediaType =NULL;
    // pMediaType =GetMediaType(pDevFilter,640L,480L,30,FORMAT_VideoInfo,MEDIASUBTYPE_YUY2);
    pMediaType =GetMediaType(pDevFilter,2592L,1544L,15,FORMAT_VideoInfo,MEDIASUBTYPE_MJPG);
    ...
    hr = pSampleGrabber->SetMediaType(pMediaType);
    ...
    cout << dec << __LINE__ << endl;

    // 连接所有的pin
    hr = pGraph->Connect(pDevCapturePin, pStInPin);
    cout << dec << __LINE__ << " - " << hex << hr << " pin " << pDevCapturePin << " " << pStInPin << endl;
    hr = pGraph->Connect(pStCapPin, pSgInPin);
    cout << dec << __LINE__ << " - " << hex << hr << endl;
    hr = pGraph->Connect(pStPrevPin, pVrInPin);
    cout << dec << __LINE__ << " - " << hex << hr << endl;
    ...
    hr = pGraph->QueryInterface(IID_IVideoWindow, (void **)&pVideoWnd);
    cout << dec << __LINE__ << " - " << hex << hr << endl;
    ...
    hr = pVideoWnd->SetWindowPosition(0, 0, rc.right, rc.bottom); // be this big
    cout << dec << __LINE__ << " - " << hex << hr << endl;
    // 设置事件通知
    hr = pEvent->SetNotifyWindow((OAHWND)hwnd, lMsg, 0);
    cout << dec << __LINE__ << " - " << hex << hr << endl;
