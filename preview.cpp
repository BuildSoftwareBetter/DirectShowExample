 ...
 * 构建默认的usb设备视频预览
 * 
 * 编译：cl preview.cpp SaveGraphFile.cpp Strmiids.lib ole32.lib OleAut32.lib
*/
#include <afxwin.h>
#include <tchar.h>
#include <dshow.h>
...
int main()
{
    ...
    // ChooseDevice(_T("CX3-UVC"), &pDeviceFilter);
    ...
    GUID pSelectVideoType = MEDIASUBTYPE_YUY2;
    LONG pSelectWidth =640L;// 1920L;
    LONG pSelectHeight =480L;// 1080L;
    int pSelectFrameRate = 30;
    ...
    while (pEnumPins->Next(1, &pPrev, NULL) == S_OK)
    {
        ...
        // if (pDir == PINDIR_OUTPUT)
        {
            ...
            // LPWSTR pp = (LPWSTR)TEXT("Preview");
            // if (StrCmpCW(pId, pp)) // 查找预览输出PIN
            // {
            //     break;
            // }
    ...
    hr = pBuild->FindInterface(&LOOK_DOWNSTREAM_ONLY, &MEDIATYPE_Video, pSmartTee, IID_IBaseFilter, (void **)&pMJPEGDecFilter);
    ShowFilterInfo(pMJPEGDecFilter);

    pMJPEGDecFilter->EnumPins(&pEnumPins);
    while (pEnumPins->Next(1, &pPrev, NULL) == S_OK)
    {
        PIN_DIRECTION pDir;
        pPrev->QueryDirection(&pDir);
        ...
        AM_MEDIA_TYPE mjpegMediaType;
        hr = pPrev->ConnectionMediaType(&mjpegMediaType);

        cout << "pin media format type \t";
        ShowGUID(&mjpegMediaType.formattype);
        ShowGUID(&mjpegMediaType.subtype); // MJPEG Decompressor Filter输入MJPG格式图像，输出RGB格式图像
    ...
    // 获取Video Renderer Filter
    IBaseFilter *pVideoRendererFilter = NULL;
    hr = pBuild->FindInterface(&LOOK_DOWNSTREAM_ONLY, &MEDIATYPE_Video, pMJPEGDecFilter, IID_IBaseFilter, (void **)&pVideoRendererFilter);
    ShowFilterInfo(pVideoRendererFilter);
    ...
    //     ULONG pins = 1;
    //     hr = pPrev->QueryInternalConnections(&pNextPin, &pins);
    //     cout << "start show next pin " << dec << pins << " return " << hex << hr << endl;
    //     // ShowPinInfo(pPrev);
    ...
    hr = pBuild->RenderStream(&PIN_CATEGORY_CAPTURE, &MEDIATYPE_Video, pSmartTee, pGrabberF, pNullFilter);
